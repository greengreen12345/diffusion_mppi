...
class IsaacGymWrapper:
    def __init__(
        ...
        self.device = device
        self.robot_indices = torch.tensor([i for i, a in enumerate(self.env_cfg) if a.type == "robot"],
                                          device=self.device)
    ...
    def reset_actor(self):
        """
        Reset all actors (cubeA, cubeB, etc.) to their initial positions and orientations
        defined in env_cfg. This mimics the same setup as creat_env, without reloading assets.
        """
        self.acquire_states()

        for actor_cfg in self.env_cfg:
            actor_idx = self._get_actor_index_by_name(actor_cfg.name)


            if actor_cfg.name == "cubeA":
                pos = actor_cfg.init_pos_on_shelf if self.cube_on_shelf else actor_cfg.init_pos_on_table
            else:
                pos = actor_cfg.init_pos

            ori = actor_cfg.init_ori

            self._root_state[:, actor_idx, 0:3] = torch.tensor(pos, device=self.device)
            self._root_state[:, actor_idx, 3:7] = torch.tensor(ori, device=self.device)
            self._root_state[:, actor_idx, 7:] = 0.0  # velocity and angular velocity

        # 应用到模拟器
        self.set_actor_root_state_tensor(self._root_state)
    ...
    def _create_actor(self, env, env_idx, asset, actor: actor_utils.ActorWrapper) -> int:
        ...
        if actor.name == "cubeA":
            pose.p = gymapi.Vec3(*actor.init_pos_on_shelf) if self.cube_on_shelf else gymapi.Vec3(
                *actor.init_pos_on_table)
        else:
    ...
    def reset(self):
        ...
        cube_state = self.get_actor_link_by_name("cubeB", "box")
    ...
    def _get_obs1(self): #reach任务
        ...
        return {
            'observation': obs.cpu().numpy().copy(),
            'achieved_goal': achieved_goal.cpu().numpy().copy(),
            'desired_goal': torch.as_tensor(self.desired_goal, device=self.device,
                                            dtype=torch.float32).cpu().numpy().copy()
        }
    ...
    def _get_obs(self):
        ...
        - CubeA position (3,)
        - CubeA relative to end-effector (3,)
        - Gripper command value (1,) (average of dof 7 and 8)
        Total dimension: 10
        """
        ...
        left_finger = self.get_actor_link_by_name("panda", "panda_leftfinger")
        right_finger = self.get_actor_link_by_name("panda", "panda_rightfinger")
        self.ee_state = ((left_finger + right_finger) / 2)[0]  # shape: (13,)

        ee_pos = self.ee_state[0:3].unsqueeze(0)  # shape: (1, 3)

        # Get the position of cubeA
        cubeA_state = self.get_actor_link_by_name("cubeA", "box")[0]
        cubeA_pos = cubeA_state[0:3].unsqueeze(0)  # shape: (1, 3)

        gripper_dist = torch.linalg.norm(left_finger[:, :3] - right_finger[:, :3], axis=1)
        gripper_state = torch.tensor([[gripper_dist]], device=self.device)

        # Concatenate observation vectors
        obs_tensor = torch.cat([ee_pos, cubeA_pos], dim=1)  # shape: (1, 7)

        achieved_goal = ee_pos.clone()
        ...
        return {
            'observation': obs_tensor.cpu().numpy().copy(),
            'achieved_goal': achieved_goal.cpu().numpy().copy(),
            'desired_goal': torch.as_tensor(self.desired_goal, device=self.device,
                                            dtype=torch.float32).cpu().numpy().copy()
        }
