...
class Objective(object):
    ...
    def compute_cost(self, sim: wrapper):
        ...
        #return 6*task_cost + 6*get_motion_cost_1 + self.get_pick_tilt_cost(sim)
        # return 5 * task_cost +self.get_pick_tilt_cost(sim) #绿方块
        #return 10 * task_cost + self.get_pick_tilt_cost(sim) #push训练配置 reach训练配置

        #return task_cost + get_motion_cost_1 #避障的训练配置
    ...
    def get_pick_tilt_cost(self, sim):
        ...
        #cubeA_ori = sim.get_actor_orientation_by_name("cubeB") #从物块A改到物块B
        # cube_quaternion = cube_state[:, 3:7]
        if not self.multi_modal:
    ...
    def get_motion_cost_11(self, sim: wrapper):
        ...
        # self.closest_dist_all = (self.closest_dist_all < 1.0).float()

        # self.closest_dist_all[self.closest_dist_all < 0.5] = 1
        # self.closest_dist_all[self.closest_dist_all > 0.5] = 0

        print(self.closest_dist_all)

        return 2000 * self.closest_dist_all
        # return 200 * self.closest_dist_all
        # return 190 * self.closest_dist_all

        # self.closest_dist_all = 1 / self.closest_dist_all
        #
        # print(self.closest_dist_all)
        #
        # return self.closest_dist_all*0.5
        # return self.closest_dist_all * 0.05
    ...
    def obs_positions1(self, sim):

        device = sim.device

        actor_names = ["table_stand", "dyn-obs", "shelf_stand", "table"]
        # actor_names = ["shelf_stand"]
        all_vertices = []
        for actor_name in actor_names:
    ...
    def distance_repulsion_nn1(self, sim, q_prev, aot=False):
        device = sim.device
        n_inputs = q_prev.shape[0]
        #print("q_prev.shape", q_prev.shape)
        self.obs = self.obs_positions(sim).to(device)
    ...
    def get_ee_navigation_cost(self, sim):
        ...
        # if self.step < 1000:
        #     self.goal = [0.2,-0.2,1.1]
        #     self.step += 1
        # self.goal = [0.6,-0.2,1.3]
        # if not isinstance(self.goal, torch.Tensor):
        #     self.goal = torch.tensor(self.goal, device=ee_pos.device, dtype=ee_pos.dtype)
        # else:
        #     self.goal = self.goal.to(device=ee_pos.device, dtype=ee_pos.dtype)
